...
/**
 * @brief   Adam Graphics Routines for texas hold'em
...
/**
 * @brief Draw the Texas Hold 'Em Logo
 */
void drawLogo()
{
...
void drawCard(unsigned char x, unsigned char y, unsigned char partial, const char* s, unsigned char isHidden)
{
    ...
    else // FULL CARD
    {
        ...
        if (s[0]=='?')
        {
            ...
            // As well as clear the second hole card, assuming a fold
            vdp_color(C_TEXT,C_TABLE,C_BORDER);
            if (x>WIDTH-3) {
                // Right seat: erase the second hole card's leftover column
                // (cards sat at x and x-2), then shift left so the back
                // fits on screen.
                for (i=0;i<5;i++) {
                    gotoxy(x-2,y+i);
                    cputc(' ');
                ...
                // Live right seat's second hole card: the first back owns
                // the right edge, so draw only this card's left column
                // peeking out from behind it - a full back here would
                // bury the first card and read as a single card.
                drawCard(x,y,PARTIAL_LEFT,s,isHidden);
                return;
            } else {
                // Erase the leftover columns of the second hole card
                // (x+3..x+4); the back redraw itself covers x..x+2.
                for (i=0;i<5;i++) {
                    gotoxy(x+3,y+i);
                    cputs("  ");
                }
