...
void initSound()
{
    // Silence all four channels. Called from initGraphics() too, before the
    // first slow appkey round-trips: the SN76489 powers up buzzing and a
    // $55AA client skips the BIOS title screen that would normally quiet it.
    sn_out(0x9F);
    sn_out(0xBF);
    sn_out(0xDF);
    sn_out(0xFF);
