...
// The community board sits at rows 9-13 (COMMUNITY_Y 9, cards 5 rows tall), so
// the pot is pushed BELOW it -- 11+POT_Y_MODIFIER is the pot box's top row, and
// -1 (the other 32-col ports' value) put it at row 10, straight under the
// board, where the turn and river cards buried it. +3 lands it at rows 14-15,
// clear of both the board above and the bottom seat below.
...
// console has 1K of RAM and the state alone is 429 bytes, so the state is
...
// src/coleco/input.c synthesises key codes from the keypad and the fire
// buttons, and src/coleco/osk.c types text on screen.
