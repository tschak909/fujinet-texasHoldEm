...
bytes. The ROM side has coleco-romstamp's fences; this is the RAM side's
equivalent: read the linker map and refuse the build if the end of BSS+DATA
rises above the line that guarantees at least ~216 bytes of stack. If this
fires, look for a new buffer that should be reading the reply window instead
(see src/misc.h's COLECO_REPLY_WINDOW rules).
"""
