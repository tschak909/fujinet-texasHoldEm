...
--
-- Adopted verbatim from fujinet-5cardstud/support/coleco/smoke.lua.
...
--
--   TEX_FONT    path to src/coleco/font.bin (required)
--   TEX_AT      emulated seconds to settle before sampling; measured from the
--               end of TEX_SCRIPT when there is one
--   TEX_SETTLE  emulated seconds before the first press (default 8)
--   TEX_SCRIPT  comma-separated controller actions to play first, e.g.
...
--   TEX_EXPECT  substring that must appear on screen; sets the exit verdict
--   TEX_QUIET   only print the verdict, not the screen

local AT     = tonumber(os.getenv("TEX_AT") or "8")
local EXPECT = os.getenv("TEX_EXPECT")
if EXPECT == "" then EXPECT = nil end   -- make/env hand through an empty string
local QUIET  = os.getenv("TEX_QUIET")
local FONT   = os.getenv("TEX_FONT")

local SCRIPT = os.getenv("TEX_SCRIPT")
...
local function load_font()
    if FONT == nil then return "TEX_FONT is not set" end
...
local SETTLE = tonumber(os.getenv("TEX_SETTLE") or "8")
...
_G.fcs_sub = emu.add_machine_frame_notifier(function ()
    ...
    if os.getenv("TEX_SNAP") then manager.machine.video:snapshot() end
