...
'
' Texas Hold'em delta from 5 Card Stud: community[11] is inserted right
' after viewing (offset 87), shifting everything from validMoveCount on
' by +11. Offsets locked by the _Static_asserts in
' support/host-test/holdem_host_test.c against src/misc.h.
'
' /state, /move/XX, /leave -> Game struct, up to 429 bytes:
'   0   lastResult[81]
'  81   round (1)              0=waiting 1=PRE-FLOP 2=FLOP 3=TURN 4=RIVER 5=SHOWDOWN
'  82   pot (u16 LE)
'  84   activePlayer (signed, $FF = -1)
'  85   moveTime (1, seconds)
'  86   viewing (1, 0/1)
'  87   community (11)         up to 5 cards x 2 chars + NUL; "" pre-flop,
'                              6/8/10 chars at flop/turn/river-showdown
'  98   validMoveCount (1)
'  99   validMoves[5] -- always 5 slots, 13 bytes each: move[3] + name[10]
' 164   playerCount (1)
' 165   players[N] -- N = playerCount, 33 bytes each:
'         name[9] status(1) bet(u16 LE) move[8] purse(u16 LE) hand[11]
'         hand holds the 2 hole cards: "????" masked, "??" folded, 4 real
'         chars at showdown (the field keeps 5 Card Stud's 11-byte width)
'
...
'   then N x 36 bytes: table[9] name[21] players[6] (literal "cur / max")
    ...
    CONST GAME_COMMUNITY      = 87
    CONST GAME_VALIDMOVECOUNT = 98
    CONST GAME_VALIDMOVES     = 99
    CONST GAME_PLAYERCOUNT    = 164
    CONST GAME_PLAYERS        = 165
    ...
    CONST PL_STATUS = 9      ' 1 byte: 0 waiting, 1 playing, 2 folded, 3 left, 4 all-in
    CONST PL_BET    = 10     ' u16 LE
    CONST PL_MOVE   = 12     ' 8 bytes
    CONST PL_PURSE  = 20     ' u16 LE
    CONST PL_HAND   = 22     ' 11 bytes (2 hole cards x 2 chars + NUL)
    ...
    CONST GAME_MAXLEN   = 429
    CONST GAME_MINLEN   = 165  ' fixed prefix through playerCount, 0 players
...
' draw_field is in texas.bas -- it renders a field straight from FN_RX (or
