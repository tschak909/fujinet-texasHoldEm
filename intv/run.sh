#!/bin/sh
# run.sh -- build (if needed) and launch Texas Hold'em in the FujiNet-patched
...
# last built.
if [ ! -f texas.rom ] || [ -n "$(find . -maxdepth 1 -name '*.bas' -newer texas.rom)" ]; then
    echo "Building texas.rom..."
    make
fi
